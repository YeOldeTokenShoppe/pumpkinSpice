...
export default class Commons {
  ...
  private createCamera() {
    this.camera = new PerspectiveCamera(
      ...
      100,  // Changed near plane from 200 to 100
      2000
    );
  ...
  private createRenderer() {
    // Check if there's an existing canvas element to use
    const existingCanvas = document.getElementById('webgl-canvas') as HTMLCanvasElement;
    
    this.renderer = new WebGLRenderer({
      canvas: existingCanvas || undefined, // Use existing canvas if available
    ...
    // Only append if we didn't use an existing canvas
    if (!existingCanvas) {
