...
const NeuralNetworkR3F = ({ 
  ...
  const handlePointerDown = (event) => {
    ...
    if (nodesRef.current && connectionsRef.current) {
      nodesRef.current.material.uniforms.uPulsePositions.value[idx].copy(intersectionPoint)
      nodesRef.current.material.uniforms.uPulseTimes.value[idx] = time
      connectionsRef.current.material.uniforms.uPulsePositions.value[idx].copy(intersectionPoint)
