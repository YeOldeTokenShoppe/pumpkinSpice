...
export const CharacterController = () => {
  ...
  useEffect(() => {
    ...
    loadSound('walking', '/sounds/bubblier_step.wav', false);
  ...
  useFrame(({ camera, mouse }) => {
    ...
    // Update character position in GameState
    const worldPos = new Vector3();
    character.current.getWorldPosition(worldPos);
    GameState.characterPosition.x = worldPos.x;
    GameState.characterPosition.y = worldPos.y;
    GameState.characterPosition.z = worldPos.z;

    // Set spawn point on first frame when character is on solid ground
    if (!hasSetSpawnPoint.current && isOnGround.current) {
      spawnPoint.current.copy(worldPos);
